...
\pnum
Each non-variant element of the aggregate that is not explicitly initialized
is initialized from its default member initializer~(\ref{class.mem}) or,
if there is no default member initializer, copy-initialized from an empty
initializer list~(\ref{dcl.init.list}).
If the aggregate is a union and the initializer list is empty, then
\begin{itemize}
\item
if any union member has a default member initilizer,
